      ...
      <footer className="bg-linear-to-br from-gray-900 via-orange-950 to-gray-900 text-white">
        ...
        <div className="max-w-7xl mx-auto px-8 py-14 grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-3 mt-2">
              {["📘", "📸", "🐦", "▶️"].map((icon, i) => (
                <button
                  key={i}
                  className="w-9 h-9 rounded-full bg-orange-500/20 border border-orange-500/30 flex items-center justify-center text-sm hover:bg-orange-500 transition-all duration-300"
                >
                  {icon}
                </button>
              ))}
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-2">
              {[
                { label: "🍕 Pizza", to: "/menu" },
                { label: "🍔 Burgers", to: "/menu" },
                { label: "🍜 Noodles", to: "/menu" },
                { label: "🍰 Desserts", to: "/menu" },
              ].map((item) => (
                <li key={item.label}>
                  <Link
                    to={item.to}
                    className="text-gray-400 text-sm hover:text-orange-400 transition duration-300"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              {[
                { icon: "📍", text: "123 Food Street, Mumbai, India" },
                { icon: "📞", text: "+91 98765 43210" },
                { icon: "✉️", text: "hello@cravings.com" },
                { icon: "🕐", text: "Mon - Sun: 9:00 AM – 11:00 PM" },
              ].map((item) => (
        ...
        <div className="border-t border-orange-500/20">
          <div className="max-w-7xl mx-auto px-8 py-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <h4 className="text-lg font-bold">🎁 Get exclusive offers!</h4>
            ...
            <div className="flex w-full md:w-auto gap-2">
              ...
              />
              <button className="btn-shimmer text-white font-bold px-6 py-2.5 rounded-xl hover:opacity-90 transition-all duration-300 text-sm whitespace-nowrap">
                Subscribe →
              </button>
            </div>
          </div>
