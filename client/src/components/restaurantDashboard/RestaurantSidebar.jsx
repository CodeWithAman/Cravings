...
const RestaurantSidebar = ({ activeTab, setActiveTab }) => {
  ...
  const renderTab = (tab) => (
    <li
      key={tab.value}
      className={`cursor-pointer p-1 rounded text-(--color-neutral) flex items-center gap-2 ${
        activeTab === tab.value
          ? "bg-(--color-primary) text-(--color-primary-content) font-semibold"
          : "hover:bg-(--color-secondary) hover:text-(--color-secondary-content) transition-colors duration-300"
      }`}
